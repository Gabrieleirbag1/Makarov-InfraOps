...
class StaffListApiView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            serializer.save()
            response = requests.get(f"http://web-rest-1:8001/users/infos/users/?username={request.data.get('user_ref')}")
            id = response.json()[0]['id']
            headers = {'Content-Type': 'application/json'}
            data = {
            'is_superuser': True
            }
            response = requests.put(f"http://web-rest-1:8001/users/staff/users/{id}/", data=json.dumps(data), headers=headers)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class StaffDetailApiView(APIView):
    ...
    def delete(self, request, id, *args, **kwargs):
        ...
        try:
            response = requests.get(f"http://web-rest-1:8001/users/infos/users/?username={infosstaff.user_ref}")
            id = response.json()[0]['id']
            headers = {'Content-Type': 'application/json'}
            data = {
            'is_superuser': False
