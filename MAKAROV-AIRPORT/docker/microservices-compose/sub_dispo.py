...
class SubscriberDispo():
    ...
    async def dispo(self, msg):
        async def get_avion(numvol):
            print(numvol)
            url = f"http://web-rest-2:8002/vols/infos/?numvol={numvol}"
        ...
        async def get_dispo(avion):
            url = f"http://web-rest-4:8004/structure/infos/avions/?modele={avion}"
            headers = {"Content-Type": "application/json"}
            response = requests.get(url, headers=headers)
            response_data = response.json()
        ...
        async def get_nb_reservations(numvol):
            url = f"http://web-rest-3:8003/reservations/infos/"
            headers = {"Content-Type": "application/json"}
            response = requests.get(url, headers=headers)
            response_data = response.json()
